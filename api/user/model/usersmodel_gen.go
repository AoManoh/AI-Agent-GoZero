...
type (
	...
	Users struct {
		Id           int64     `db:"id"`            // 用户的唯一标识符 (主键)
		Username     string    `db:"username"`      // 用户的登录名 (唯一)
		PasswordHash string    `db:"password_hash"` // 存储经过哈希和加盐处理后的用户密码
		CreatedAt    time.Time `db:"created_at"`    // 用户账户的创建时间戳
	}
)
...
func (m *defaultUsersModel) Update(ctx context.Context, newData *Users) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.Id, newData.Username, newData.PasswordHash)
	return err
}
